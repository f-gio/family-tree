const ISO_ALPHA_2_TO_3 = Object.freeze({
...
export function toIsoAlpha3(countryCode) {
  const code = clean(countryCode).toUpperCase();
  if (/^[A-Z]{3}$/.test(code)) return code;
  return ISO_ALPHA_2_TO_3[code] || "";
}

/**
 * Formate un lieu pour un contexte de lecture rapide sans enrichir les
 * anciennes valeurs texte. Le code pays n'est ajouté que lorsqu'il provient
 * de métadonnées structurées fiables.
 */
export function formatCompactPlace(text = "", placeInfo = null) {
  const legacyText = clean(text);
  if (!placeInfo || typeof placeInfo !== "object") return legacyText;
  const name = clean(placeInfo.name) || legacyText;
  if (!name) return "";
  const countryCode = toIsoAlpha3(placeInfo.countryCode);
